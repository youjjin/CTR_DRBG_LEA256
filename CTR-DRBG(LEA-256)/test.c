...
{
	STATE state;
	state.state_control.prediction_resisitance_flag = 1;
	state.state_control.df_flag = 1;

	unsigned char pseudorandom_bits[128] = { 0x00, };
	...
	fp_req = fopen("CTR_DRBG(LEA-256(use DF)(use PR))_KAT.req", "r");
	fp_fax = fopen("CTR_DRBG(LEA-256(use DF)(use PR))_KAT.rsp", "w");
	...
	for (j = 0; j < 4; j++)
	{
		...
		for (i = 0; i < 15; i++)
		{
			...
			memset(buff, 0, sizeof(buff));
			/****************HASH_DRBG***************/
			usePR(&state, Entropy, entropy_len, Entropy1, entropy1_len, Entropy2, entropy2_len, Nonce, nonce_len, Personalizationstring, personalization_len, Additional1, additionalinput1_len, Additional2, additionalinput2_len, pseudorandom_bits);
			/*****************MAC***************/
			printf("ReturnedBits = ");
...
void LEA_DRBG_LEA256_usePR_noDF_Test()
{
	...
	fp_req = fopen("CTR_DRBG(LEA-256(no DF)(use PR))_KAT.req", "r");
	fp_fax = fopen("CTR_DRBG(LEA-256(no DF)(use PR))_KAT.rsp", "w");
...
void LEA_DRBG_LEA256_noPR_useDF_Test()
{
	...
	fp_req = fopen("CTR_DRBG(LEA-256(use DF)(no PR))_KAT.req", "r");
	fp_fax = fopen("CTR_DRBG(LEA-256(use DF)(no PR))_KAT.rsp", "w");
	...
	for (j = 0; j < 4; j++)
	{
		...
		for (i = 0; i < 15; i++)
		{
			...
			/****************HASH_DRBG***************/
			noPR(&state, Entropy, entropy_len, Nonce, nonce_len, Personalizationstring, personalization_len, Entropy1, entropy1_len, Entropy2, entropy2_len, Additional1, additionalinput1_len, Additional2, additionalinput2_len, pseudorandom_bits);
...
void LEA_DRBG_LEA256_noPR_noDF_Test()
{
	...
	fp_req = fopen("CTR_DRBG(LEA-256(no DF)(no PR))_KAT.req", "r");
	fp_fax = fopen("CTR_DRBG(LEA-256(no DF)(no PR))_KAT.rsp", "w");
